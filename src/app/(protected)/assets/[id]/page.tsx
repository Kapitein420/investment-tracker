...
export default async function AssetDetailPage(props: { params: Promise<{ id: string }> }) {
  ...
  // These five reads are independent of one another, so fire them
  // concurrently instead of awaiting in series (previously ~5 sequential
  // round-trips). The asset query still drives the notFound() check below.
  const [asset, stages, users, companies, contents] = await Promise.all([

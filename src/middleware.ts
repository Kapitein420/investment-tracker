...
export default withAuth(
  ...
  },
  {
    callbacks: {
      ...
      // Those additionally reject tokens the jwt callback has marked
      // invalidated (deactivated account, or password rotated since the
      // token was issued) — a public path never carries that check, since
      // it does not gate on the token at all.
      authorized: ({ req, token }) =>
        !isProtected(req.nextUrl.pathname) ||
        (!!token && !(token as any).invalidated),
    },
  }
);

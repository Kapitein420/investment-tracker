...
| **BW 3:15a / 6:227a + eIDAS Art. 25** (e-sign, e-contract) | Yes | NDA and NBO signed in-app | "Sufficiently reliable" method; provable identity, integrity, moment of conclusion | Partly — signer IP/UA + PDF hash persisted (#172); certificate, copy-to-signer, intent checkbox still open — §9 G8 |
...
| G8 · Thin e-signature evidence | Partly | #172 | `signerIp`/`signerUserAgent`/`pdfSha256` now persisted at signing (`prisma/schema.prisma:454-458`; `src/actions/document-actions.ts:976-977,1021-1022,1181`); `ActivityLog.userId` is now nullable (`schema.prisma:407`), fixing the misattributed-actor problem structurally. Still missing: completion certificate, copy-to-signer email, intent checkbox. No `feat/signing-certificate` branch exists on `origin` — that work has not started. |
...
| G13 · Portal terms of use | Open | — | Investor footer still only links the external "Algemene voorwaarden" page (`src/components/investor/investor-shell.tsx:79-84`); no in-app click-accept exists. |
| G14 · Free-text PII risk | Partly | #170 | The BSN elfproef guard (`src/lib/validators.ts`) remains the only check; no IBAN/passport pattern guard was added. No `feat/free-text-pii-guards` branch exists on `origin` — that work has not started. |
